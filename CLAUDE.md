...
- **BGA project name:** `minirailsmospinach` · **SFTP remote path:** `/minirailsmospinach/` (in the gitignored `.vscode/sftp.json`). `minirails` was already taken — by the abandoned prior project for this game. The name also fixes the stylesheet filename (`minirailsmospinach.css`), the PHP namespace, and the `Gamehelpminirailsmospinach` wiki page BGA renders as the in-game "How to play?" panel. **Read the real namespace out of BGA's generated skeleton rather than deriving it** — the studlycasing of a run-together name is BGA's to decide, not ours to guess. If the `minirails` name is ever freed or transferred, renaming is only clean **before** alpha; after that an admin has to do it.
