/**
 * Type definitions for BGA's bga-zoom component library — DEV ONLY.
 *
 * Not our code and not deployed: the lib itself is loaded from BGA's CDN at runtime by
 * src/ts/libs.ts, and this file exists purely so TypeScript can check the calls. It is excluded
 * from the SFTP allowlist in scripts/deploy.mjs along with the other bga-*.d.ts stubs.
 *
 * BGA publishes it at
 *   https://x.boardgamearena.net/data/game-libs/bga-zoom/1.x/dist/bga-zoom.d.ts
 * (see https://en.doc.boardgamearena.com/Zoom). This copy came from the studio's own build in
 * ../_reference/castlecombo/, which is the same published stub. Re-download from BGA if the lib
 * gains options this file does not know about.
 */
